package TermExtract::MeCabVerbAll;
...
sub get_noun_frq {
    ...
    my @verbterms = ();         # 動詞用（Yoshida edit）

    my $previous_pos = 0;       # 動詞用（Yoshida edit）
    my $previous_noun = '';       # 動詞用（Yoshida edit）
	...
	my ($noun, $values) = (split /\t/, $morph);
		...
		if (scalar(@terms) > 0 && $previous_noun eq 'サ変接続') {
			push @terms, $noun;
		} else {
	...
	# 動詞用（Yoshida edit） start
	if(scalar(@terms) < 1 ) {

		if ($part_of_speach eq '動詞') {
			if($cl_1 eq '非自立') {
				my $lastverb = pop @verbterms;
				@verbterms = ();
				push @verbterms, $lastverb;
				push @verbterms, $noun;
			} else {
				@verbterms = ();
			...
			if ($previous_pos == 1 && ($noun eq 'たい' || $noun eq 'ない')) {
			...
			if (scalar(@verbterms) > 1) {

		       		&$add(\@verbterms, \%cmp_noun_list);
